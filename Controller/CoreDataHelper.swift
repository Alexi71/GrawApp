...
class CoreDataHelper {
    ...
    static func insertFlightData(flight : FlightData, dataArray : [InputData]) {
        let context = getContext()
        let flightData = Ascent(entity: Ascent.entity(), insertInto: context)
        flightData.key = flight.key
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "MM/dd/yyyy HH:mm:ss"
        dateFormatter.timeZone = TimeZone(identifier: "UTC")
        flightData.date = dateFormatter.date(from: "\(flight.date) \(flight.time)") as NSDate!
        flightData.dataUrl = flight.url
        flightData.temp100Url = flight.url100
        flightData.tempEndUrl = flight.urlEnd
        ...
        for item in dataArray {
            ...
            data.geopotential = item.geopotential
            data.altitude = item.altitude
            data.latitude = item.latitude
            data.longitude = item.longitude
            data.windspeed = item.windSpeed
            data.winddirection = item.windDirection
            data.dewpoint = item.dewpoint
            flightData.addToFlightHasData(data)
        }
        
        save()
    }
    
    static func isFlightStored(key : String) ->Bool{
        
        if let context = getContext() {
            let fetchRequest = Ascent.fetchRequest() as NSFetchRequest<Ascent>
            fetchRequest.predicate = NSPredicate(format: "key ==%@", key)
            if let flightData = try? context.fetch(fetchRequest)  {
    ...
    static func getFlightData(key : String) ->[Ascent]?{
